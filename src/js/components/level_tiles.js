...
export default class LevelTiles extends Object2D {
  ...
  sprite_ready(sprites) {
    ...
    return this.emit_signal(LevelTiles.LEVEL_DONE)
  };

  generate_tiles() {
    return this.level_loop((options) => {
      ...
      return this.emit_signal(LevelTiles.SPRITE_GEN, s_id, sprite.position)
    })
  };
  ...
  update_position(scale) {
    ...
    return this.update_global_position()
  };
  ...
  update_scale(scale) {
    ...
    this.level_loop((options) => {
      ...
      sprite.scale = scale;
      return sprite.position = new Vector2(options.dimension.vec.x * this._t_grotto.size.x * scale, options.dimension.vec.y * this._t_grotto.size.y * scale)
    });

    return this.update_position(scale)
  };

  level_loop(callback) {
    return this._layers.forEach((ld, il) => {
      if (ld.type === LevelTiles.TILE_LAYER) {
        return Level.dimension_loop(this.parent.dimension, (vec, i) => {
