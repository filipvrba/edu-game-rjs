...
export default class UI extends BasicObject {
  ...
  ready() {
    ...
    return this._scenes.next_scene(Scenes.IDLE_C, -1)
  };
  ...
  click() {
    ...
    if (scene.event) return scene.event()
  };

  write_message(str) {
    if (str) {
      this._panel.style.visibility = "visible";
      return this._panel.innerHTML = `<p>${str}</p>`
    } else {
      return this._panel.style.visibility = "hidden"
    }
  }
};
